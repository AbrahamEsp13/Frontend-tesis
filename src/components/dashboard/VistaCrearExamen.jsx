    ...
    <div className="grid grid-cols-1 md:grid-cols-5 gap-12 items-start">
      ...
      <div className="md:col-span-2 space-y-6 pt-4">
        ...
        <p className="text-lg text-gray-600 leading-relaxed">
          Sube tus apuntes o presentaciones. Nuestra Inteligencia Artificial analizará el contenido y creará una evaluación personalizada en cuestión de segundos.
        </p>
        ...
        <div className="space-y-5 pt-4">
          <div className="flex items-center gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
            ...
            <p className="font-semibold text-gray-700 m-0">Dale un nombre y categoría</p>
          </div>
          <div className="flex items-center gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
            ...
            <p className="font-semibold text-gray-700 m-0">Carga tu documento PDF base</p>
          </div>
          <div className="flex items-center gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
      ...
      <div className="md:col-span-3 space-y-8">
        ...
        {/* 0. NUEVOS INPUTS: NOMBRE Y MATERIA */}
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          ...
          <div className="mb-6">
            ...
            <input 
              ...
              className="w-full p-4 border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all text-gray-700 font-medium bg-gray-50/50"
              disabled={cargando}
            />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <select 
                ...
                className="w-full p-4 border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none transition-all text-gray-700 font-medium bg-gray-50/50 appearance-none cursor-pointer"
                disabled={cargando}
              >
        ...
        <div className="bg-white p-10 rounded-3xl shadow-lg border-2 border-dashed border-gray-300 flex flex-col items-center text-center group hover:border-blue-400 hover:bg-blue-50/50 transition-all cursor-pointer relative">
          <span className="material-symbols-outlined text-7xl text-gray-400 group-hover:text-blue-500 transition-colors mb-5">cloud_upload</span>
          ...
          <input type="file" accept=".pdf" onChange={manejarCambioArchivo} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" id="fileUpload" disabled={cargando} />
          {!nombreArchivoMostrado && (
             <label htmlFor="fileUpload" className="mt-6 py-3 px-8 bg-gray-100 group-hover:bg-blue-600 text-gray-700 group-hover:text-white font-bold rounded-full transition-colors cursor-pointer shadow-sm text-sm">Seleccionar archivo</label>
          )}
        </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          ...
          <div className="space-y-3">
            <label className="block font-bold text-gray-800 text-lg">Cantidad de preguntas a generar:</label>
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-4 mt-2">
              <div className="flex items-center bg-gray-50 border border-gray-300 rounded-xl w-48 sm:w-52 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all shrink-0">
                <input type="number" min="1" max="15" value={numPreguntas} onChange={(e) => setNumPreguntas(e.target.value)} className="w-full p-4 rounded-l-xl border-none text-2xl outline-none text-center font-extrabold text-gray-700 bg-transparent" disabled={cargando} />
                <span className="px-4 text-gray-500 font-bold border-l border-gray-200 text-sm shrink-0">preguntas</span>
              </div>
              <p className="text-sm text-gray-500 m-0 leading-tight">Recomendado: 5 a 15 reactivos para mayor precisión.</p>
            </div>
          </div>
        </div>
        
        {/* BOTÓN DE GENERAR (REVISA QUE ESTÉ TODO) */}
        <button 
          onClick={generarCuestionario} 
          disabled={cargando || !nombreArchivoMostrado || !nombreExamen.trim() || !materia} 
          className={`w-full py-5 px-6 text-xl font-bold text-white border-none rounded-2xl shadow-xl transition-all flex justify-center items-center gap-3 outline-none shadow-blue-500/20 ${cargando || !nombreArchivoMostrado || !nombreExamen.trim() || !materia ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 cursor-pointer hover:-translate-y-0.5'}`}
        >
